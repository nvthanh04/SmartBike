...
class _UserMainScreenState extends State<UserMainScreen> {
        ...
        content: Column(
          ...
          children: [
            ...
            if (data['duration'] != null)
              _detailRow(Icons.timer_outlined, "Thời gian", "${data['duration']} phút", Colors.orange),
            if (data['distance'] != null)
              _detailRow(Icons.route, "Quãng đường", "${data['distance']} km", Colors.teal),
            const Divider(height: 20),
            _detailRow(Icons.payments_outlined, "Chi phí", "${data['cost'] ?? 0}đ", Colors.redAccent, bold: true),
            const SizedBox(height: 8),
            Container(
  ...
  Widget build(BuildContext context) {
   ...
   void _showMockScanQRDialog(BuildContext context) async {
    String userId = FirebaseAuth.instance.currentUser?.uid ?? "user_test";
    final paymentService = PaymentService(); // ✅ Sửa: PaymentService() thay vì _paymentService
    ...
    builder: (context) => AlertDialog(
      ...
      content: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          const Text("✓ Tìm thấy xe BIKE-001",
            ...
            child: ElevatedButton(
              ...
              onPressed: () {
                ...
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) =>
                        const ActiveTripScreen(bikeId: 'BIKE-001'),
                  ),
                );
              },
              child: const Text(
